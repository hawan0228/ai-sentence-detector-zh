...
這個專案是一個中文 AI 文字風格分析的研究與展示，比較兩種二元分類方法：
...
> 本工具估計的是文字與訓練語料的風格相似程度，不能證明作者是否真的使用 AI。
> 這個專案僅為練習，沒有任何商樣行為。
...
- 使用「較偏向人類／無法確定／較偏向 AI」三段式結果，僅限做為參考。
...
資料由本人從不同平台蒐集，並自行透過 prompt 調適與 prompt engineering 產生及整理 AI 文本。AI 語料使用過的模型系列包括：
...
兩個步驟以 scikit-learn `Pipeline` 儲存成單一模型。系統也會計算輸入中實際出現的 n-gram 對分類分數的正負貢獻
